...
import { createOrFetchConversation } from "../../redux/slices/chatSlice"; // Import the createOrFetchConversation action
...
export default function JobProposals() {
  ...
  // Function to create or fetch a conversation and navigate to the chat page
  const handleStartConversation = async (engineerId) => {
    try {
      const token = localStorage.getItem("Token");
      const decodedToken = jwtDecode(token); // Assuming you use jwtDecode for getting the user's ID from the token
      const userId = decodedToken?.id;

      // Dispatch the action to create or fetch a conversation between the current user and the engineer
      const conversationResponse = await dispatch(
        createOrFetchConversation({ senderId: userId, receiverId: engineerId })
      ).unwrap();

      // Once the conversation is created or fetched, navigate to the chat page with the receiver's ID
      navigate(`/chat/${engineerId}`);
    } catch (err) {
